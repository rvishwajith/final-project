...
public class OceanGeometryController : MonoBehaviour
{
    ...
    [SerializeField] bool showMaterialLods;
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < wavesGenerator.cascades.Length; i++)
        {
            var suffix = "_c" + i;
            oceanMaterial.SetTexture("_Displacement" + suffix, wavesGenerator.cascades[i].Displacement);
            oceanMaterial.SetTexture("_Derivatives" + suffix, wavesGenerator.cascades[i].Derivatives);
            oceanMaterial.SetTexture("_Turbulence" + suffix, wavesGenerator.cascades[i].Turbulence);
        }
